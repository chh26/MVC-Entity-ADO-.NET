    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            using (var uow = UnitOfWorkFactory.Create(connString))
            {
                ...
                user = userRepo.GetSysUser(account, Password);
            ...
            if (user.name == null)
            {
                ModelState.AddModelError("AccPWErrMsg", "您輸入的帳號不存在！");
                return View(model);
            }

            if (user.password != model.Password)
            {
                ModelState.AddModelError("AccPWErrMsg", "密碼錯誤！");
                return View(model);
            }
        ...
        public async Task<ActionResult> ResetPassword(ResetPasswordViewModel model)
        {
            ...
            try
            {
                ...
                {
                    var sysUserRepo = new OperatorRepository(uow);
                    user = sysUserRepo.GetSysUser(account, oldPassword);
            ...
            catch (Exception)
            {
                ...
                ModelState.AddModelError("checkOldPasswordErr", "確認原密碼時，發生錯誤！");
                return View(model);
            }
            ...
            {
                ModelState.AddModelError("AccPWErrMsg", "原密碼輸入錯誤！");
                return View(model);
            }
            ...
            try
            {
                ...
                {
                    var sysUserRepo = new OperatorRepository(uow);
                    isChanged = sysUserRepo.UpdateSysUserPW(account, newPassword);

                    if (isChanged)
                        sysUserRepo.SaveChanges();
                    else
                    {
                        sysUserRepo.Dispose();
                        ModelState.AddModelError("ResetErr", "密碼更新錯誤，請洽管理者！");
                        return View(model);
                    }
            ...
            {
                ModelState.AddModelError("ResetErr", "確認原密碼時，發生錯誤！!");
                return View(model);
            }
